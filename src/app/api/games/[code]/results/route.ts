...
export async function GET(req: Request, { params }: { params: Promise<{ code: string }> }) {
  return handle(async () => {
    ...
    const [{ data: questions, error: qErr }, { data: players, error: pErr }, { data: votes, error: vErr }] =
      await Promise.all([
    ...
    if (qErr || pErr || vErr) throw qErr ?? pErr ?? vErr;
    ...
    return json({ status: game.status, playerCount: players?.length ?? 0, submitted, tallies });
  });
}

...
export async function PUT(req: Request, { params }: { params: Promise<{ code: string }> }) {
  return handle(async () => {
    ...
    const { questionId, guesserId, guessedId } = await readJson<{ questionId?: string; guesserId?: string; guessedId?: string | null }>(req);
    ...
    const { error } = await db
      .from("guesses")
      .upsert({ game_id: game.id, question_id: questionId, guesser_id: guesserId, guessed_id: guessedId ?? null }, { onConflict: "question_id,guesser_id" });
